...
export default function PopupNoticeRegion({ notices = [] }) {
  ...
  const visibleNotices = useMemo(
    () => notices.filter((notice) => !dismissed.has(notice.id)).slice(0, 3),
    [dismissed, notices],
  );
  useEffect(() => {
    if (mobileIndex >= visibleNotices.length)
      setMobileIndex(Math.max(0, visibleNotices.length - 1));
  ...
  return (
    <section
      className="pointer-events-none fixed inset-x-0 bottom-4 z-40 px-4 sm:bottom-6"
      aria-label={language === "ko" ? "팝업 공지" : "Popup notices"}
      aria-live="polite"
    >
      <div className="mx-auto flex max-w-6xl justify-end">
        <div className="pointer-events-auto hidden gap-4 md:flex">
          {visibleNotices.map((notice) => (
            <PopupNoticeCard
              key={notice.id}
              notice={notice}
              onClose={() => hide(notice, "session")}
              onDayDismiss={() => hide(notice, "day")}
            />
          ))}
        </div>
        <div className="pointer-events-auto flex w-full flex-col gap-3 md:hidden">
          <PopupNoticeCard
            notice={current}
            onClose={() => hide(current, "session")}
            onDayDismiss={() => hide(current, "day")}
          />
          {visibleNotices.length > 1 ? (
            <div className="flex items-center justify-between rounded-full border border-slate-200 bg-white/95 px-4 py-2 text-xs text-[var(--bw-color-ink)] shadow-[var(--bw-shadow-soft)]">
              <button
                type="button"
                onClick={() =>
                  setMobileIndex(
                    (index) =>
                      (index - 1 + visibleNotices.length) %
              ...
              <span>
                {mobileIndex + 1}/{visibleNotices.length}
              </span>
              <button
                type="button"
                onClick={() =>
                  setMobileIndex((index) => (index + 1) % visibleNotices.length)
                }
              >
                {language === "ko" ? "다음" : "Next"}
              </button>
