...
export default function NoticesPage({ data, categories, query }: NoticesProps) {
  ...
  return (
    <div className="min-h-screen bg-[var(--bw-color-surface-muted)] text-[var(--bw-color-ink)]">
      ...
      <SeoMetadata
        title={
          language === "ko" ? "공지사항 | 브레인웍스" : "Notices | Brainworks"
        }
        description={
          language === "ko"
            ? "브레인웍스의 공식 공지사항입니다."
            : "Official notices from Brainworks."
        }
      />
        ...
        <div className="mx-auto max-w-6xl px-6 py-12">
          ...
          >
            <label className="sr-only" htmlFor="notice-search">
              {language === "ko" ? "공지사항 검색" : "Search notices"}
            </label>
            <input
              id="notice-search"
              name="q"
            ...
            <button
              className="min-h-11 rounded-full bg-[var(--bw-color-ink)] px-6 text-sm font-semibold text-white transition hover:bg-black"
              type="submit"
            >
...
export async function getServerSideProps({
  ...
  const normalized = {
    ...
    categoryId: typeof query.category === "string" ? query.category : "",
    page: Number(query.page) || 1,
  };
  const [data, categories] = await Promise.all([
    getPublishedNoticeList(currentLocale, normalized),
    getPublishedNoticeCategories(currentLocale),
  ]);
