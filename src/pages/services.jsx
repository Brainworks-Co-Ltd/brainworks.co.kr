...
export default function Services() {
  ...
  const localizedAreas = useMemo(
    () => getLocalizedBusinessAreas(language),
    [language],
  );
  ...
  const activeArea =
    localizedAreas.find((area) => area.id === activeId) ?? localizedAreas[0];

  const getTabClasses = (isActive) => {
    const base =
      "whitespace-nowrap border-b-2 pb-3 text-base font-medium transition-colors duration-200 ";
    return isActive
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main id="main-content">
        <section className="relative overflow-hidden border-b border-slate-200 bg-gradient-to-b from-white via-sky-50 to-slate-100">
          <div
            className="absolute inset-y-0 right-[-40%] hidden rotate-6 rounded-full bg-sky-500/10 blur-3xl md:block"
            aria-hidden="true"
          />
          ...
          <div className="container mx-auto px-4">
            <div className="rounded-3xl border border-slate-200 bg-gradient-to-r from-sky-100 via-white to-slate-100 p-12 text-center shadow-xl">
              <h3 className="text-3xl font-semibold">
                {language === "ko"
                  ? "맞춤형 AI 프로젝트를 함께 시작해보세요"
                  : "Let’s launch your tailored AI initiative"}
              </h3>
