...
export async function getServerSideProps(context: GetServerSidePropsContext) { const guard = await requireAdminPage(context); if ("redirect" in guard) return guard; const id = typeof context.params?.popupNoticeId === "string" ? context.params.popupNoticeId : ""; const notice = await getAdminPopupNotice(id); return { props: { notice: { id: notice.id, version: notice.version, locales: notice.locales.map((locale) => ({ locale: locale.locale, title: locale.title })) } } }; }
